    ...
    <div className="border border-zinc-800 rounded-xl bg-zinc-900/20 overflow-hidden transition-all hover:border-zinc-700 shadow-sm group">
      ...
      >
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <span className={clsx(
            'text-sm font-medium truncate transition-colors max-w-[200px] sm:max-w-xs',
            item.question ? 'text-zinc-200' : 'text-zinc-500 italic'
          )}>
             ...
             <div ref={wizardScrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#18181b]">
                {wizardMessages.map((msg) => (
                  <div key={msg.id} className={clsx(
                    ...
                    <div className={clsx(
                      'max-w-[85%] rounded-2xl px-5 py-3 text-[16px] leading-relaxed whitespace-pre-line',
                      msg.role === 'user' ? 'bg-[#3f3f46] text-white' : 'text-zinc-200'
                    )}>
                    ...
                    <div className="flex items-end p-2 gap-2">
                        ...
                        <div className="flex items-center gap-1 pb-1 pr-1">
                            ...
                            className={clsx(
                              ...
                              isListening ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30 animate-pulse' : 'text-zinc-400 hover:text-white hover:bg-zinc-700'
                            )}
                            >
                            <Mic size={22} />
                            </button>
                            ...
                            disabled={(!wizardInput.trim() && wizardFiles.length === 0) || isWizardLoading}
                            className={clsx(
                              'p-2.5 rounded-full transition-all duration-200 flex items-center justify-center',
                              (wizardInput.trim() || wizardFiles.length > 0) ? 'bg-white text-black hover:bg-zinc-200' : 'bg-[#3f3f46] text-zinc-500 cursor-not-allowed'
                            )}
                            >
                            <ArrowRight size={22} />
                            </button>
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className={clsx(
                  'flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold transition-all border',
                  config.active ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500'
                )}
              >
                 <Power size={18} />
          ...
          <div className="space-y-10">
            ...
            <div className="pt-8 pb-12">
              <button 
                ...
                disabled={isSaving}
                className={clsx(
                  'w-full py-4 rounded-xl text-lg font-semibold transition-all flex items-center justify-center gap-3 shadow-xl',
                  isSaving ? 'bg-emerald-600 text-white cursor-wait' : 'bg-white text-black hover:bg-zinc-200 active:scale-[0.99] shadow-white/5'
                )}
              >
                {isSaving ? (
                   <>
                 ...
                 {previewMessages.map((msg) => (
                    <div key={msg.id} className={clsx(
                      ...
                      msg.role === 'user' ? 'justify-end' : 'justify-start'
                        ...
                        <div className={clsx(
                          'px-4 py-2.5 max-w-[85%] text-[13px] leading-snug',
                          msg.role === 'user' ? 'bg-indigo-600 text-white rounded-2xl rounded-tr-sm' : 'bg-zinc-800 text-zinc-200 rounded-2xl rounded-tl-sm'
                        )}>
                          {msg.text}
                        </div>
                    </div>
