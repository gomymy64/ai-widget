...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, isOpen, onClose, isDemoMode }) => {
  return (
    <aside 
      className={`
        ...
        <div className="p-6 border-t border-zinc-800/50">
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-900/50 rounded-xl p-4 border border-zinc-800 shadow-xl">
            <h4 className="text-sm font-semibold text-white mb-1">
              {isDemoMode ? 'Тариф FREE' : 'Тариф PRO активен'}
            </h4>
            <p className="text-xs text-zinc-500 mb-3">
              {isDemoMode ? 'Ограниченные возможности' : 'Списание: 24 Окт'}
            </p>
            <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
              <div 
                className={`h-full rounded-full ${isDemoMode ? 'bg-zinc-500 w-[10%]' : 'bg-electric-500 w-3/4'}`}
              ></div>
            </div>
            <p className="text-[10px] text-zinc-500 mt-2 text-right">
              {isDemoMode ? '10% Лимит' : '75% Лимит'}
            </p>
          </div>
        </div>
      </div>
